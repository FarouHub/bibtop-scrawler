...
let BibScrawler = (function () {
            ...
            if (!error && response.statusCode == 200) {
                ...
                articles['root'].forEach(function(detail) {
                    if(typeof detail.url != 'undefined') {
                        ...
                        setTimeout(function(){
                            context.parser_article(navInfoArticle, detail, itemHandler, function(){timeToFinish--;})
                        }, _timeinterval*index);
                    }else if(typeof detail.homeInfo != 'undefined'){
                        itemHandler(detail, null, function(){});
                    }
                    index++;
                });
     ...
     * @param {*} callback 
     */
    BibScrawler.prototype.parser_article = function(navInfo, homeInfo, itemHandler, callback) {
        ...
        request(options, function (error, response, html) {
            if (!error && response.statusCode == 200) {
                ...
                itemHandler(homeInfo, item.root[0], callback);
            }else{
